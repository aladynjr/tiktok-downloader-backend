...
(async () => {
    ...
    console.log(chalk.green("\nTotal videos:", media.length));
        //got list of all videos in media

    if (!fs.existsSync(`videos`)) fs.mkdirSync(`videos`)
    if (!fs.existsSync(`videos/${username}`)) fs.mkdirSync(`videos/${username}`)

    //if videos folder doesnt exist, create it
    ...
    //link variable will get the link of the video
        
        console.log(`[${i}] ${link}`)
        let filename = link.split("/").pop()+".mp4";
        //filename variable will get the name of the video using video id (example : 6820000000000000000.mp4) )
        ...
        //getNowm variable will get the no watermark video link
